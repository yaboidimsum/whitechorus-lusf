    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-coral/50 focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-plum shadow-xl will-change-transform",
                  cardClassName,
                )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-2 top-1/2 z-[200] -translate-y-1/2 rounded-2xl border border-cream/20 bg-plum/70 p-2.5 text-cream shadow-sm backdrop-blur-md transition hover:border-coral/50 hover:bg-plum/90 active:scale-95 sm:left-4"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-2 top-1/2 z-[200] -translate-y-1/2 rounded-2xl border border-cream/20 bg-plum/70 p-2.5 text-cream shadow-sm backdrop-blur-md transition hover:border-coral/50 hover:bg-plum/90 active:scale-95 sm:right-4"
            >
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[230px] text-[12px]">
              {active.meta.map((row) => (
                <div key={row.label} className="flex justify-between py-[5px] border-b border-cream/10">
        ...
        <div className="mt-4 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2 rounded-full bg-cream transition-opacity",
                index === selected ? "opacity-100 scale-125" : "opacity-30",
              )}
            />
          ))}
        </div>
